...
def onlyDeviceTypesSupportProfilingTest(fn):
    ...
    def only_fn(self, *args, **kwargs):
        if self.device_type not in DEVICE_LIST_SUPPORT_PROFILING_TEST:
            reason = f"onlyNativeDeviceTypesForProfiling: can't run on {self.device_type}. If the device supports profiler and plans to test, please add the device into the DEVICE_LIST_SUPPORT_PROFILING_TEST"
            raise unittest.SkipTest(reason)
